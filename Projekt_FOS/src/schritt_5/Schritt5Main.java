...
public class Schritt5Main
{
	...
	//Ließt alle Textdateien in einem Pfad aus und gibt sie an die untere Methode weiter um sie auszulesen
			...
			{	
				if(fileArray.length!=0)
				{
					...
					for(int i=0;i<fileArray.length;i++) //Schleife, die ein Array ausgibt, in der Pfade und Dateien gespeichert sind.
					{
						...
						if((fileArray[i].isFile())&&(fileArray[i].getName().endsWith(".txt"))) //Prüft ob es eine Datei ist und es der Dateityp ist
						{
	...
	public static void Dateiauslesen(String pfad,String suchWort)
	{
		...
		System.out.println(); //Für die übersichtlichkeit
	}
